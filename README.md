...
- 이미지 모드 1차 코어:
  ...
  - 이미지 다운로드 자동화 1차
    ...
    - 이벤트를 못 잡아도 기본 다운로드 위치의 새 파일을 찾아 사용자 저장 폴더로 옮기는 폴백 포함
    - 작업 뒤에는 프로젝트 탭 외 보조 탭을 자동으로 정리
    - 검색 결과 첫 타일을 무작정 쓰지 않고, 원본 Flow Classic Plus처럼 태그/카드 구조/위치 점수로 맞는 카드를 우선 선택
    - 검색 기반 다운로드가 끝나면 검색어를 다시 지워 전체 프로젝트 화면으로 복귀
    - CDP 세션으로 브라우저 다운로드 경로를 사용자 저장 폴더로 직접 잡으려고 시도
