...
- 이미지 모드 1차 코어:
  ...
  - 사람처럼 프롬프트 타이핑
  - 프롬프트 머리글 정규화
    - `S001>S002 Prompt : ...` 도 실제 입력은 `S001 Prompt : ...`
    - `V001>V002 Prompt : ...` 도 실제 입력은 `V001 Prompt : ...`
  ...
  - 생성 후 남은 시간 실시간 안내
  - 이미지 다운로드 자동화 1차
    - 카드에서 태그가 바로 안 보여도 다운로드 검색창에 `S002` 같은 태그를 넣고
    - 검색 결과의 첫 타일 기준으로 다운로드를 이어가는 폴백 포함
    - 검색창 입력 직후 즉시 `Enter`를 치지 않고 결과 반영을 먼저 기다리도록 보정
    - 다운로드 이벤트는 현재 페이지뿐 아니라 브라우저 컨텍스트 전체 기준으로 감지
  - 실행마다 `logs/action_trace_*.log` 액션 트레이스 저장
  - 대기열 카드를 여러 칸으로 배치해 한 줄에 여러 개 표시
